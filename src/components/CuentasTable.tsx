...
// Nota: Es mejor práctica tener estos tipos en un archivo central (ej: src/types/index.d.ts)
// y luego importarlos aquí para evitar duplicados.
enum StatusCuenta { 
    ACTIVO = "ACTIVO", 
    VENCIDO = "VENCIDO", 
...
export const CuentasTable = ({ 
    ...
    // Estado para manejar el filtro por tipo de cuenta en las pestañas "Vendidas" y "Stock"
    const [tipoFilter, setTipoFilter] = useState<'all' | TipoCuenta>('all');
    ...
    const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.checked) {
            const accountsToSelect = cuentas
    ...
    // Hook useMemo para filtrar y ordenar las cuentas eficientemente.
    const processedCuentas = useMemo(() => {
        // Aplicar la lógica especial solo a las pestañas "Vendidas" y "Stock"
        if (activeTab !== 'sold' && activeTab !== 'stock') {
            return cuentas;
        }

        // 1. Filtrar por el tipo de cuenta seleccionado (funciona para ambas pestañas)
        const filtered = cuentas.filter(cuenta => {
            if (tipoFilter === 'all') return true;
            return cuenta.tipoCuenta === tipoFilter;
        });

        // 2. Ordenar la lista filtrada según la pestaña activa
        if (activeTab === 'sold') {
            return filtered.sort((a, b) => {
                const aIsFull = a.tipoCuenta === TipoCuenta.INDIVIDUAL && a.perfilesOcupados >= a.perfilesMaximos;
                const bIsFull = b.tipoCuenta === TipoCuenta.INDIVIDUAL && b.perfilesOcupados >= b.perfilesMaximos;
                if (aIsFull !== bIsFull) {
                    return aIsFull ? 1 : -1; // Prioriza las que tienen perfiles disponibles
                }
                return new Date(b.fechaInicio).getTime() - new Date(a.fechaInicio).getTime(); // Luego por fecha
            });
        }

        if (activeTab === 'stock') {
            // Para "Stock", solo ordenar por fecha de creación (más reciente primero)
            return filtered.sort((a, b) => {
        ...
        <div>
            {selectedAccounts.length > 0 && activeTab === 'stock' && (
                <motion.div 
                    ...
                    exit={{opacity: 0, y: -20}} 
                    className="mb-4 bg-slate-700 p-3 rounded-lg flex justify-between items-center"
                >
                    <span className="font-semibold">{selectedAccounts.length} cuenta(s) completa(s) seleccionada(s)</span>
                    <button 
                        onClick={() => onOpenModal('bulk-sell', null)} 
            ...
            <div className="bg-slate-800/50 border border-slate-700 rounded-xl overflow-x-auto">
                <table className="min-w-full text-sm text-left">
                        ...
                        <tr>
                            ...
                            <th scope="col" className="px-4 py-3">
                                <div className='flex items-center gap-2'>
                                    <span>TIPO</span>
                                    {/* El filtro por tipo ahora aparece en "Vendidas" y "Stock" */}
                                    {(activeTab === 'sold' || activeTab === 'stock') && (
                                        <select
                                            value={tipoFilter}
                                            onChange={(e) => setTipoFilter(e.target.value as 'all' | TipoCuenta)}
                                            onClick={(e) => e.stopPropagation()} // Evita que se disparen otros eventos de la cabecera
                                            className="bg-slate-700 border-none rounded text-xs p-1 focus:ring-1 focus:ring-blue-500"
                                        >
                                            <option value="all">Todos</option>
                                            <option value={TipoCuenta.INDIVIDUAL}>Individual</option>
                                            <option value={TipoCuenta.COMPLETO}>Completo</option>
                                        </select>
                                    )}
                                </div>
                            </th>
                    ...
                    <tbody className="text-slate-200">
                        {/* Se itera sobre la lista ya procesada (filtrada y ordenada) */}
                        {processedCuentas.map((account, index) => {
                            ...
                            return (
                                <tr key={account.id} className={`border-b border-slate-700 transition-colors ${selectedAccounts.includes(account.id) ? 'bg-blue-900/50' : 'hover:bg-slate-800'}`}>
                                    ...
                                    <td className="px-4 py-4">
                                        <span className={`px-2 py-1 text-xs font-semibold rounded-full ${getStatusBadge(account.status)}`}>{account.status}</span>
                                    </td>
                                    <td className="px-4 py-4">
                                        <span className={`px-2 py-1 text-xs font-semibold rounded-full ${account.tipoCuenta === 'INDIVIDUAL' ? 'bg-purple-500/20 text-purple-300' : 'bg-orange-500/20 text-orange-300'}`}>{account.tipoCuenta}</span>
                                    </td>
                                    <td className="px-4 py-4">
